...
data rel1;
    set n2309.a_rel (keep = stysid1a RLPCFM1C DAY_1N FLAG_REL RELTM1);
...
data demo;
    set n2309.a_dmg (keep = stysid1a TRTREG1C SEX1C RCE5C age_1n_cat);
*	format RCE5C rce_.;
run;

data ae;
    set n2309.a_aeinf (keep = stysid1a SOC_ABBR SOC_TXT AEVSER1C AEVSTT1D DYSTT_1N);
...
data visit;
    set n2309.a_vis (keep = stysid1a dyvis_1n visnam1a);
run;
...
data death;
    set n2309.a_cmp (keep = stysid1a dydth_1n dcnrsn1c);
	where dcnrsn1c = 10;
    death = 1;
...
data discont_ae;
    set n2309.a_cmp (keep = stysid1a dcnrsn1c dylst_1n);
	where dcnrsn1c = 1;
    discont_ae = 1;
...
data edss;
    set n2309.a_edss (keep = stysid1a FCDPG3N1 TCDPG3N1);
	format _all_;
run;
...
data mri1;
    set n2309.a_mri (keep = stysid1a DAYNI_1N MRIRSL1A RSLVAL1N TVOLT2CH);
...
data edss2;
    set n2309.a_edss (keep = stysid1a FCDPG3N1 TCDPG3N1 DAYNI_1N);
	format _all_;
run;
...
data a_cmp;
    set n2309.a_cmp (keep = stysid1a dcnrsn1c dystt_1n LSTSTR1D);
	where dcnrsn1c = 1;
run;

data a_aeinf;
    set n2309.a_aeinf (keep = stysid1a ACNTAK3N AEVSTT1D AEVEND1D);
    where ACNTAK3N = 2;
	format _all_;
run;
