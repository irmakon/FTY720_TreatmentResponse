...
data rel1;
    set n2302.a_rel (keep = stysid1a RLPCFM1C DAYNI_1N FLAG_REL RELTM1);
...
data demo;
    set n2302.a_dmg (keep = stysid1a TRTREG1C SEX1C RCE5C age_1n_cat);
*	format RCE5C rce_.;
run;

data ae;
    set n2302.a_aeinf (keep = stysid1a SOC_ABBR SOC_TXT AEVSER1C AEVSTT1D DYSTT_1N);
...
data visit;
    set n2302.a_vis (keep = stysid1a dyvis_1n visnam1a);
run;
...
data death;
    set n2302.a_cmp (keep = stysid1a dydth_1n dcnrsn1c);
	where dcnrsn1c = 10;
    death = 1;
...
data discont_ae;
    set n2302.a_cmp (keep = stysid1a dcnrsn1c dylst_1n);
	where dcnrsn1c = 1;
    discont_ae = 1;
...
data edss;
    set n2302.a_edss (keep = stysid1a FCDPG3N1 TCDPG3N1);
	format _all_;
run;
...
data mri1;
    set n2302.a_mri (keep = stysid1a DAYNI_1N MRIRSL1A RSLVAL1N TVOLT2CH FRET2ITT);
	where MRIRSL1A = "T2_NWEN" and RSLVAL1N > 0 and DAYNI_1N > 0; 
...
data edss2;
    set n2302.a_edss (keep = stysid1a FCDPG3N1 TCDPG3N1 DAYNI_1N);
	format _all_;
run;
...
data a_cmp;
    set n2302.a_cmp (keep = stysid1a dcnrsn1c dystt_1n LSTSTR1D);
	where dcnrsn1c = 1;
run;

data a_aeinf;
    set n2302.a_aeinf (keep = stysid1a ACNTAK3N AEVSTT1D AEVEND1D);
    where ACNTAK3N = 2;
	format _all_;
run;
...
data outcome;
	retain stysid1a DayLastVisit aevser1c sae_startday outcome_sae censor_sae day_sae
		   ...
		   RLPCFM1C FLAG_REL dayni_1n RELTM1 outcome_relaps censor_relaps day_relaps
	...
	by stysid1a;
	*Serious AE;
    if sae_startday < 406 and aevser1c = 1 then do;
        outcome_sae = 1;
		censor_sae = 1;
		day_sae = sae_startday;
	end;
	else do;
        if DayLastVisit > 405 then do;
		    censor_sae = 0;
    		day_sae = 405;
		end;
		else do;
	...
	*Infection;
    if infec_startday < 406 and infec = 1 then do;
        outcome_infec = 1;
		censor_infec = 1;
	...
	*Neoplasm;
    if neopl_startday < 406 and neopl = 1 then do;
        outcome_neopl = 1;
		censor_neopl = 1;
	...
	*Relaps;
    if RELTM1 < 406 and RELTM1 ne . then do;
        outcome_relaps = 1;
		censor_relaps = 1;
		day_relaps = RELTM1;
	end;
	...
	*Death;
    if death = 1 and dydth_1n < 406 then do;
        outcome_death = 1;
		censor_death = 1;
		day_death = dydth_1n;
	end;
	...
	*Discontinued due to AE;
    if discont_ae = 1 and discont_day < 406 then do;
	...
	*T2 new/enlarged;
    if t2_startday > 0 and t2_startday < 406 then do;
        outcome_t2new = 1;
		censor_t2new = 1;
	...
	*outcome #11;
	...
	drop aevser1c sae_startday 
		 ...
		 RLPCFM1C FLAG_REL dayni_1n RELTM1 
         dydth_1n death 
