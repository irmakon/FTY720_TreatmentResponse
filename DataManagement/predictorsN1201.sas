libname h "D:\Users\mse0jxh8\Documents\My SAS Files\9.4\N1201";
...
data demo;
    set n1201.a_dmg_mse (keep = stysid1a TRTREG1C SEX1C BMI_1N RCE5C age_1n);
	rename sex1c = Sex bmi_1n = BMI rce5c = Race age_1n = Age;
	format RCE5C rce_.;
run;

data ident;
    set n1201.a_ident_mse (keep = stysid1a trtn priorms TGPDSC1A);
	rename priorms = PriorMSDr trtn = Drug;
run;

data edss;
    set n1201.a_edss_mse (keep = stysid1a vis1n EXDSC1NB FUNBOW1N FUNBRA1N FUNCER1N FUNMEN1N FUNPYR1N
                             FUNSEN1N FUNVSU1N);
...
data mri;
    set n1201.a_mri_mse (keep = vis1n stysid1a NUMGD1BA);
	where vis1n = 1; *screening;
	MRI_VolGdT1 = .;
    MRI_VolT2 = .;
    MRI_VolHypT1 = .;
	rename NUMGD1BA = MRI_NumGdT1;
run;
...
data mshis;
    set n1201.a_mshis_mse (keep = stysid1a vis1n DURMS1 DURMS1C NUMRLP2N RELTIME RCTRLP1D VIS1D);
	where vis1n = 1 and NUMRLP2N ne .; *screening;
	rename DURMS1 = DisDur NUMRLP2N = RlpsNo_2y RELTIME = RlpsDist;
	if substr(RCTRLP1D,1,2) = "--" then do;
	    RCTRLP1D = cat("01",substr(RCTRLP1D,3,7));
 	end;
   	rlp_dt = input(RCTRLP1D, date10.);
	vis_dt = input(VIS1D, date10.);
	diff = vis_dt - rlp_dt;
	RlpDistC = diff / 30;
	format _all_;
	drop rlp_dt vis_dt diff;
run;
...
data mshis2;
    set n1201.a_mshis_mse (keep = STYSID1A vis1n prvtmt8c prvtmt3c);
	where vis1n = 1 and prvtmt8c ne .;
	format _all_;
run;
...
data mshis2_t;
	...
	label PriorMSDr_IB = "Prior Interferon beta-1 treatment"
		  ...
		  ;
	drop _NAME_ _LABEL_ _1 _3 _88;
run;
/*
data lrs;
    set n1201.a_lrs (keep = stysid1a vis1n blflg_1c CV_BL_1N PARNAM1C CVUNT_1C LABCAT1C);
...
data lrs99;
    set n1201.a_lrs (keep = stysid1a CV_BL_1N PARNAM1C);
...
data unique;
    set n1201.a_lrs (keep = PARNAM1C CVUNT_1C LABCAT1C);
	format _all_;
run;
...
*/
data lrs;
    set n1201.a_lrs_mse (keep = stysid1a CV_BL_1N PARNAM1C CVUNT_1C LABCAT1C);
	where CV_BL_1N ne . and 
	      PARNAM1C in ("ABSBAS","ABSEOS","ABSLYM","ABSMON","ABSNEU","ALB","ALKPHS","CREA","DBIL",
                       "GGT","SGOT","SGPT","TBIL","UPROTST","WBC");
	format _all_;
run;
...
%macro renamelab(ds);
    data &ds.2;
    set &ds.;
    %let lab_list = ABSBAS ABSEOS ABSLYM ABSMON ABSNEU ALB ALKPHS CREA DBIL GGT SGOT SGPT TBIL UPROTST WBC
                    ;
	%let b=1;
...
data lrs_ct3;
	length ABSBAS ABSEOS ABSLYM ABSMON ABSNEU ALB ALKPHS CREA DBIL GGT SGOT SGPT TBIL UPROTST WBC $10.;
...
/*
data qol;
    set n1201.a_qol_mse (keep = stysid1a vis1n pag1a ACT4C ANX1C MOB1C PAI4C SFC1C VASBL);
	where vis1n = 1 and pag1a ="013"; *screening;
...
data msfc;
    set n1201.a_msfc_mse (keep = stysid1a VIS1N HPTBL PASATBL T25FWBL);
...
/*
data cnd99;
    set n1201.a_cnd (keep = stysid1a SOC_ABBR ACTPRB1C SOC_TXT); * SOC_CODE = "" fuer alle;
...
*/
data cnd;
    set n1201.a_cnd_mse (keep = stysid1a SOC_ABBR ACTPRB1C SOC_TXT); * SOC_CODE = "" fuer alle;
	where SOC_ABBR ne "";
	format _all_;
run;
...
data cm;
    set n1201.a_cmdatc_mse (keep = stysid1a CONCTIM ATCCODE);
