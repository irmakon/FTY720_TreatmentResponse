...
data rel1;
    set n1201.a_rel_mse (keep = stysid1a RLPCFM1C DAY_1N FLAG_REL RELTM1);
...
data demo;
    set n1201.a_dmg_mse (keep = stysid1a TRTREG1C SEX1C RCE5C age_1n);
*	format RCE5C rce_.;
run;

data ae;
    set n1201.a_aeinf_mse (keep = stysid1a SOC_ABBR SOC_TXT AEVSER1C AEVSTT1D DAYNI_1N);
	where SOC_TXT ne "" and DAYNI_1N > 0;               *DAYNI_1N missing = General disorder...;
...
data visit;
    set n1201.a_vis_mse (keep = stysid1a dyvis_1n visnam1a);
run;
...
data death;
    set n1201.a_cmp_mse (keep = stysid1a dydth_1n dcnrsn1c);
	where dcnrsn1c = 10;
    death = 1;
...
data discont_ae;
    set n1201.a_cmp_mse (keep = stysid1a dcnrsn1c dylst_1n);
	where dcnrsn1c = 1;
    discont_ae = 1;
...
/*
data edss;
    set n1201.a_edss_mse (keep = stysid1a FCDPG3N1 TCDPG3N1);
	format _all_;
run;
...
*/
data mri1;
    set n1201.a_mri_mse (keep = stysid1a DAYNI_1N NUMNNT2);
	where NUMNNT2 > 0 and DAYNI_1N > 0; 
	format _all_;
run;
...
/*
data edss2;
    set n1201.a_edss_mse (keep = stysid1a FCDPG3N1 TCDPG3N1 DAYNI_1N);
	format _all_;
run;
...
data a_cmp;
    set n1201.a_cmp (keep = stysid1a dcnrsn1c dystt_1n LSTSTR1D);
	where dcnrsn1c = 1;
run;

data a_aeinf;
    set n1201.a_aeinf (keep = stysid1a ACNTAK3N AEVSTT1D AEVEND1D);
    where ACNTAK3N = 2;
	format _all_;
run;
...
data outcome;
	...
	by stysid1a;
	*Serious AE;
    if sae_startday < 226 and aevser1c = 1 then do;
        outcome_sae = 1;
		censor_sae = 1;
		day_sae = sae_startday;
	end;
	else do;
        if DayLastVisit > 225 then do;
		    censor_sae = 0;
    		day_sae = 225;
		end;
		else do;
	...
	*Infection;
    if infec_startday < 226 and infec = 1 then do;
        outcome_infec = 1;
		censor_infec = 1;
	...
	*Neoplasm;
    if neopl_startday < 226 and neopl = 1 then do;
        outcome_neopl = 1;
		censor_neopl = 1;
	...
	*Relaps;
    if RELTM1 < 226 and RELTM1 ne . then do;
        outcome_relaps = 1;
		censor_relaps = 1;
		day_relaps = RELTM1;
	end;
	...
	*Death;
    if death = 1 and dydth_1n < 226 then do;
        outcome_death = 1;
		censor_death = 1;
		day_death = dydth_1n;
	end;
	...
	*Discontinued due to AE;
    if discont_ae = 1 and discont_day < 226 then do;
	...
	*T2 new/enlarged;
    if t2_startday > 0 and t2_startday < 226 then do;
        outcome_t2new = 1;
		censor_t2new = 1;
...
*	if outcome_edss=1 or outcome_relaps=1 or outcome_7=1 then do;
	if outcome_relaps=1 or outcome_7=1 then do;
	    outcome_11 = 1;
		censor_11 = 1;
*		day_11 = min(day_edss, day_relaps, day_7);
		day_11 = min(day_relaps, day_7);
	end;
	else do;
*	    censor_11 = censor_edss;   *=censor_relaps=censor_7;
	    censor_11 = censor_relaps;   *=censor_relaps=censor_7;
*		day_11 = day_edss;         *=day_relaps=day_7;
		day_11 = day_relaps;         *=day_relaps=day_7;
		if day_11 > 135 then do;
		    outcome_11 = 0;
		end;
		else do;
		    outcome_11 = .;
	...
	drop sex1c rce5c age_1n trtreg1c visnam1a;
