libname h "D:\Users\mse0jxh8\Documents\My SAS Files\9.4\N2309";
...
data demo;
    set n2309.a_dmg (keep = stysid1a TRTREG1C SEX1C BMIBL_1N RCE5C age_1n_cat);
	rename sex1c = Sex bmibl_1n = BMI rce5c = Race age_1n_cat = Age;
	format RCE5C rce_.;
	label age_1n_cat = "Age category";
run;

data ident;
    set n2309.a_ident (keep = stysid1a trtn priorms TGPDSC1A);
	rename priorms = PriorMSDr trtn = Drug;
run;

data edss;
    set n2309.a_edss (keep = stysid1a vis1n EXDSC1NB FUNBOW1N FUNBRA1N FUNCER1N FUNMEN1N FUNPYR1N
                             FUNSEN1N FUNVSU1N);
...
data mri;
    set n2309.a_mri (keep = vis1n stysid1a NUMGD1BA TVOLGDBA TVOLT2BA TVT1HIBA);
...
data mshis;
    set n2309.a_mshis (keep = stysid1a vis1n DURMS1 NUMRLP2N RELTIME);
...
data mshis2;
    set n2309.a_mshis (keep = STYSID1A vis1n prvtmt8c prvtmt3c);
	where prvtmt8c ne .;
	format _all_;
run;
...
/*
data lrs;
    set n2309.a_lrs (keep = stysid1a vis1n blflg_1c CV_BL_1N PARNAM1C CVUNT_1C LABCAT1C);
...
data lrs99;
    set n2309.a_lrs (keep = stysid1a CV_BL_1N PARNAM1C);
...
data unique;
    set n2309.a_lrs (keep = PARNAM1C CVUNT_1C LABCAT1C);
	format _all_;
run;
...
*/
data lrs;
    set n2309.a_lrs (keep = stysid1a CV_BL_1N PARNAM1C CVUNT_1C LABCAT1C);
	where CV_BL_1N ne . and 
	      PARNAM1C in ("ABSBAS","ABSEOS","ABSLYM","ABSMON","ABSNEU","ALB","ALKPHS","CREA","DBIL",
                       "GGT","MCH","MCV","SGOT","SGPT","TBIL","UPROTST","WBC");
	format _all_;
...
data qol;
    set n2309.a_qol (keep = stysid1a vis1n pag1a ACT4C ANX1C MOB1C PAI4C SFC1C VASBL);
	where vis1n = 2 and pag1a ="4001"; *baseline;
...
data msfc;
    set n2309.a_msfc (keep = stysid1a VIS1N HPTBL PASATBL T25FWBL);
	where vis1n = 2; *baseline;
...
/*
data cnd99;
    set n2309.a_cnd (keep = stysid1a SOC_ABBR ACTPRB1C SOC_TXT);
...
run;                        *-> es gibt SOC_ABBR mit ACTPRB1C = 0 und missing pro Patient:;
*Pat. 568 Gastr mit 0 und .;
*/
data cnd;
    set n2309.a_cnd (keep = stysid1a SOC_ABBR ACTPRB1C SOC_TXT);
	where SOC_ABBR ne "";
...
data cm;
    set n2309.a_cmdatc (keep = stysid1a CONCTIM1 ATCCODE);
	where ATCCODE ne "" and CONCTIM1 = 1;
	atc = substr(ATCCODE,1,1);
	format _all_;
run;

proc sort data = cm out = cm2 NODUPKEY;
    by stysid1a atc CONCTIM1;
run;
...
data cm3;
    set cm2;
    by stysid1a atc CONCTIM1;
	if last.atc then output;       *ueberfluessig wenn nur CONCTIM1 = 1;
	keep stysid1a atc CONCTIM1;
run;
...
data cm4;
	...
	if in_cm = 0 and CONCTIM1 = . then CONCTIM1 = 0;
run;
